<template>
  <div>
    <PageHeader
      title="Tambah Kempen"
      subtitle="Cipta kempen sumbangan baharu."
      :breadcrumb="[{ label: 'Kempen', to: '/admin/campaigns' }, { label: 'Tambah' }]"
    >
      <template #actions>
    ...
    <div class="mt-6 max-w-2xl rounded-2xl bg-white p-6 shadow-sm ring-1 ring-gray-100">
      <form class="space-y-4" @submit.prevent="handleSubmit">
        ...
        <div>
          ...
          />
          <p class="mt-1 text-xs text-gray-400">JPG, PNG atau WEBP. Maksimum 4MB. Cadangan: Saiz poster Instagram (1350px H x 1080px W / nisbah 4:5).</p>
          <p v-if="errors.image" class="mt-1 text-xs text-red-500">{{ errors.image }}</p>
        </div>

        <div>
          <label class="block text-sm font-medium text-gray-700">Galeri Kempen (Pilihan)</label>
          <div class="mt-1.5 grid grid-cols-3 gap-2">
            <div v-for="(preview, index) in galleryPreviews" :key="index" class="relative overflow-hidden rounded-xl ring-1 ring-gray-200 aspect-square">
              ...
              <button type="button" @click="removeGalleryImage(index)" class="absolute top-1 right-1 rounded-full bg-red-500 p-1 text-white hover:bg-red-600">
        ...
        <div class="flex items-center gap-3 pt-2">
          <Button type="submit" :loading="submitting">
            {{ submitting ? 'Menyimpan...' : 'Simpan Kempen' }}
          </Button>
...
async function handleSubmit() {
  ...
  try {
    ...
    toast.error('Gagal mencipta kempen.')
  } finally {
