<template>
  <div>
    <PageHeader
      title="Tambah Banner"
      subtitle="Cipta banner baharu untuk halaman utama."
      :breadcrumb="[{ label: 'Banner', to: '/admin/banners' }, { label: 'Tambah' }]"
    >
      <template #actions>
    ...
    <div class="mt-6 max-w-2xl rounded-2xl bg-white p-6 shadow-sm ring-1 ring-gray-100">
      <form class="space-y-4" @submit.prevent="handleSubmit">
        ...
        <div>
          ...
          />
          <p class="mt-1 text-xs text-gray-400">JPG, PNG atau WEBP. Maksimum 4MB. Cadangan: Saiz 1920px H x 1080px W.</p>
...
async function handleSubmit() {
  ...
  try {
    ...
    toast.success('Banner berjaya dicipta.')
    router.push('/admin/banners')
  } catch (e) {
    if (e.response?.data?.errors) {
      Object.assign(errors, e.response.data.errors)
    }
    toast.error('Gagal mencipta banner.')
  } finally {
    submitting.value = false
  }
}
