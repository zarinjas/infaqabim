  ...
  <div>
    ...
    <template v-else>
      <div class="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <StatCard label="Donasi Belum Selesai" :value="stats?.pending_donations ?? 0" :icon="Icons.clock" color="amber" />
        <StatCard label="Donasi Selesai" :value="stats?.completed_donations ?? 0" :icon="Icons.check" color="purple" />
      </div>

      <div class="mt-6 rounded-2xl bg-white shadow-sm ring-1 ring-gray-100">
        <div class="flex items-center justify-between border-b border-gray-100 px-5 py-4">
          <h2 class="text-sm font-semibold text-gray-900">Donasi Terkini</h2>
          <router-link to="/admin/donations" class="text-xs font-medium text-emerald-600 hover:text-emerald-700">
            Lihat semua &rarr;
          </router-link>
        </div>
        <EmptyState v-if="!stats?.recent_donations?.length" title="Belum ada donasi." />
          ...
          <li v-for="d in stats.recent_donations" :key="d.id" class="flex items-center justify-between gap-4 px-5 py-3.5">
            <div class="flex min-w-0 items-center gap-3">
              ...
              <div class="min-w-0">
                ...
                <p class="truncate text-xs text-gray-400">{{ d.campaign || 'General' }} &middot; {{ d.created_at }}</p>
              </div>
            </div>
