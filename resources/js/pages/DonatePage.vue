        ...
        <div class="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-gray-100 sm:p-8">
          ...
          <form class="mt-6 space-y-5" @submit.prevent="submit">
            <FormSelect
              ...
              label="Pilih Kempen"
              :placeholder="loadingCampaigns ? 'Memuatkan...' : 'Pilih kempen'"
...
const errors = reactive({
  campaign_id: '', donor_name: '', donor_email: '', donor_phone: '', amount: '', receipt_image: '', general: '',
})
...
function validate() {
  clearErrors()
  let valid = true
  if (!form.campaign_id) { errors.campaign_id = 'Sila pilih kempen'; valid = false }
...
async function submit() {
  ...
  try {
    ...
    Object.assign(form, { campaign_id: '', donor_name: '', donor_email: '', donor_phone: '', amount: null })
...
onMounted(async () => {
  try {
    ...
    campaigns.value = home.campaigns || []
    publicSettings.value = settings || {}
    form.payment_method = availableMethods.value[0]?.value || 'manual'

    const preselect = route.query.campaign
    if (preselect && campaigns.value.some((c) => String(c.id) === String(preselect))) {
      form.campaign_id = Number(preselect)
    }
  } catch {
    campaigns.value = []
  } finally {
